...
public class StoreRestTemplateConfiguration {
    ...
    @Autowired
    private StoreApiFallbackFactory storeApiFallbackFactory;
    @Bean
    @Scope("prototype")
    ...
    public InvocationHandlerFactory mythFeignCircuitBreakerInvocationHandler() {
        return (target, dispatch) -> {
            return new MythFeignCircuitBreakerInvocationHandler(factory, target, dispatch, storeApiFallbackFactory);
        };
    }
}
